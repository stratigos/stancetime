...
import { FETCH_STANCES, SET_STANCES } from "../constants/action_types";
...
const backendApiMiddleWare = ({ dispatch }) => next => action => {
  ...
  if( [ FETCH_STANCES ].indexOf(action.type) === -1 ) {
    next(action);
    return;
  }
      ...
      if (response.status >= 300) {
        ...
        response.json()
          .then(response  => {
            ...
            dispatch( backendApiDone() );
            dispatch( { type: payload.success, payload: { nextAction: SET_STANCES, data: response.data } } );
